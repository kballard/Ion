...
local function controlOnEvent(self, event, ...)
	...
	if (event == "ADDON_LOADED" and ... == "Ion") then
		...
		if (GDB.sbarFirstRun) then
			...
			local bar, object = ION:CreateNewBar("stancebar", 1, true)

			for i=1,ION.maxStanceID do
				object = ION:CreateNewObject("stancebar", i)
			...
			for id,data in pairs(sbarsGDB) do
				if (data ~= nil) then
					ION:CreateNewBar("stancebar", id)
				end
			end

			for id,data in pairs(sbtnsGDB) do
				if (data ~= nil) then
					ION:CreateNewObject("stancebar", id)
				end
			end
		end
